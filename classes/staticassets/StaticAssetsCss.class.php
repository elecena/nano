...
class StaticAssetsCss extends StaticAssetsProcessor
{
    ...
    private function processFile(string $file): string
    {
        ...
        $content = preg_replace_callback('#@import url\(["\']?([^)]+.css)["\']?\);#', [$this, 'importCssCallback'], $content);
        ...
        if (!$this->inDebugMode()) {
            ...
            $content = preg_replace_callback('#url\(["\']?([^)]+.(gif|png))["\']?\)#', [$this, 'embedImageCallback'], $content);

            $content = strtr(trim($content), [
                '; ' => ';',
                ': ' => ':',
                ' {' => '{',
                '{ ' => '{',
        ...
        // fix relative paths
        $content = preg_replace_callback('#url\(["\']?([^)]+.)["\']?\)#', [$this, 'fixRelativeImagePath'], $content);
